...
export async function signup(
  ...
  password: string,
  userName?: string
): Promise<{ tenant: Record<string, unknown>; user: Record<string, unknown> }> {
  const slug =
    tenantName
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
  ...
  const userRows = await sql`
    ...
    VALUES (${tenant.id}::uuid, ${email}, ${passwordHash}, ${userName ?? tenantName}, 'owner', ARRAY['read', 'write', 'admin'])
...
export async function registerClient(opts: {
  ...
  const clientId = `herald_${crypto.randomBytes(16).toString("hex")}`;
...
export async function exchangeAuthorizationCode(
  ...
  if (
    ...
    !verifyCodeChallenge(codeVerifier, authCode.code_challenge as string)
  ) {
    return null;
  }

  // Mark as used
  await sql`UPDATE oauth_authorization_codes SET used_at = NOW() WHERE code = ${code}`;

  // Create tokens
  const accessToken = await createAccessToken({
    userId: authCode.user_id as string,
    tenantId: authCode.tenant_id as string,
    scope: authCode.scope as string,
    clientId,
  });
  ...
  await sql`
    ...
    VALUES (${refresh.hash}, ${clientId}, ${authCode.user_id}::uuid, ${authCode.scope}, ${expiresAt.toISOString()})
  `;
...
export async function exchangeRefreshToken(
  ...
  await sql`UPDATE oauth_refresh_tokens SET revoked_at = NOW() WHERE token_hash = ${hash}`;
  ...
  await sql`
    INSERT INTO oauth_refresh_tokens (token_hash, client_id, user_id, scope, expires_at)
    VALUES (${newRefresh.hash}, ${clientId}, ${token.user_id}::uuid, ${token.scope}, ${expiresAt.toISOString()})
  `;
...
export function isAllowedAutoRegisterDomain(redirectUri: string): boolean {
  try {
    ...
    return ALLOWED_DCR_DOMAINS.some(
      (domain) =>
